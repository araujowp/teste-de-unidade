...
public class LeilaoTest {
	...
	public void deveReceberUmLance() {
		...
		Leilao leilao = new Leilao("kombi 0 km");
		assertEquals(0, leilao.getLances().size());

		leilao.propoe(new Lance(new Usuario("Alexandra Daddario"), 2300));
		
		assertEquals(1, leilao.getLances().size());
		assertEquals(2300, leilao.getLances().get(0).getValor(), 0.00001);
	...
	public void deveReceberVariosLances() {
		...
		Leilao leilao = new Leilao("kombi 0 km");
		leilao.propoe(new Lance(new Usuario("Alexandra Daddario"), 2300));
		leilao.propoe(new Lance(new Usuario("Alexandra espósito"), 2400));
		
		assertEquals(2, leilao.getLances().size());
		assertEquals(2300, leilao.getLances().get(0).getValor(), 0.00001);
		assertEquals(2400, leilao.getLances().get(1).getValor(), 0.00001);
